...
const ASK_TOOLS = [
  { name: "query_logs",
    description: "Run one read-only SQLite SELECT (aggregate only: GROUP BY / COUNT / SUM, never raw record rows). Tables: v_logs(system_id, seq, ts_client, ts_server, category, action, payload_json) and v_warnings(system_id, severity, title, detail, status, created_at). Timestamps are ISO 8601 strings; compare as text, group a day with substr(ts_client,1,10). Returns { queryIndex, columns, rows }; reference queryIndex in present.\n\nPAYLOAD SHAPES — check which one you have before trusting a field name. Most entries come from the FileMaker OnWindowTransaction trigger and look like {file, table, record_id, data}, where `data` is the per-table calculation's output held VERBATIM. When that calculation builds JSON, `data` is a JSON STRING, so one json_extract only returns the string: you must extract twice to reach a value inside it, e.g. json_extract(json_extract(payload_json,'$.data'),'$.account'). The person who made a change usually lives in there (account, z_Modifier, modified_by), NOT at the top level. Top-level `account_name` exists only when FileMaker sent it alongside. Edits also carry `changed` (a map of field -> {from,to}), and bulk entries carry {bulk:true, count, table, op}. Chassis-style entries are the other shape, with account_name/message/rows at the top level and one json_extract. If a query returns all NULLs, you almost certainly used one extract where two were needed; retry before concluding a field is empty, and never report 'always null' without having tried the nested form.",
